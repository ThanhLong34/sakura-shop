...
require("../../classes/mails/ResetPasswordAdminMail.php");
...
function resetPassword($email)
{
   ...
   if (performsQueryAndResponseToClient($query)) {
      ...
      // Tạo đối tượng mail
      $mail = new ResetPasswordAdminMail($email, $newPassword);
...
// Lấy id tài khoản admin từ email
function checkEmailExist($email)
{
   global $connect;

   $query = "SELECT `id` FROM `admin` WHERE `email` = '$email' AND `deletedAt` IS NULL LIMIT 1";
   $result = mysqli_query($connect, $query);
