...
function getAll($limit, $offset, $searchType, $searchValue, $fillType, $fillValue, $orderby, $reverse)
{
   ...
   $baseQuery = "SELECT `$tableName`.*,
      `image`.`link` AS 'imageUrl', 
      `video`.`link` AS 'videoUrl', 
      `advertisementType`.`name` AS 'advertisementTypeName'
      FROM `$tableName`
      LEFT JOIN `image` ON `image`.`id` = `$tableName`.`imageId`
      LEFT JOIN `video` ON `video`.`id` = `$tableName`.`videoId`
      LEFT JOIN `advertisementType` ON `advertisementType`.`id` = `$tableName`.`advertisementTypeId`
